...
Instance show_tern_tree {A} `{_ : Show A} : Show (TernaryTree A) :=
    {| show := let fix aux t :=
        match t with
          | TLeaf => "TLeaf" 
          | TNode x a b c => concat " " ["TNode ("; show x; ") (";
           aux b ;") (" ;aux a ; ") (" ;aux c ;")"]
        end 
    in aux
|}.
Check (2 + 2 = 4).
(*Print (TNode 1 (TNode 2 TLeaf TLeaf TLeaf) (TLeaf) (TLeaf)).*)
(** [] *)
...
Definition liftM4 {m : Type -> Type} {M : Monad m} {A B C D E : Type} (f : A -> B -> C -> D -> E) : m A -> m B -> m C -> m D -> m E :=
  fun x y z t => bind x (fun x => liftM3 (f x) y z t).

Print liftM4.

(*Polymorphic liftM4@{t u m n} = *)
(*  fun (m : Type -> Type) (M : Monad m) (A B C D E : Type) (f : A -> B -> C -> D -> E)
   (a : m A) (b : m B) (c : m C) (d : m D) (e : m E) =>*)

(*fun bind a (fun x0 : A => bind b (fun x1 : B => bind c (fun x2 : C => bind d (fun x3 : D => ret (f x0 x1 x2 x3)))).*)

Fixpoint genTernTree {A} (sz : nat) (g : G A) : G (TernaryTree A) :=
    match sz with
      | O => ret TLeaf 
      | S sz' =>
          oneOf [
            ret TLeaf ;
            liftM4 TNode g (genTernTree sz' g) (genTernTree sz' g) (genTernTree sz' g)]
    end.

Sample (genTernTree 3 (choose(0,3))). 

(** The following line should generate a bunch of nat ternary trees. *)
 Sample (@genTernTree nat 3 (choose (0,10))). 
(** [] *)
