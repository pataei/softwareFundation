...
(**
     = fun P Q : Prop =>
       conj (fun x : P /\ Q => match x with
                               | conj x0 x1 => conj x1 x0
                               end) (fun x : Q /\ P => match x with
                                                       | conj x0 x1 => conj x1 x0
                                                       end)
...
Definition conj_fact_cheat : forall P Q R, P /\ Q -> Q /\ R -> P /\ R.
  intros. destruct H. split. apply H. destruct H0. apply H2. Defined.
(** [] *)

Compute conj_fact_cheat.

(*
     = fun (P Q R : Prop) (x : P /\ Q) (x0 : Q /\ R) =>
       match x with
       | conj x1 _ => conj x1 match x0 with
                              | conj _ x4 => x4
                              end
       end
...
Definition conj_fact P Q R (H : P /\ Q) (H' : Q /\ R) := 
  match H with
  | conj HP HQ => conj HP match H' with
                                           | conj HQ' HR => HR
                                           end
  end.
...
(*
 = fun (P Q R : Prop) (H : P /\ Q) (H' : Q /\ R) =>
       match H with
       | conj x _ => conj x match H' with
                            | conj _ x2 => x2
                            end
       end
     : forall P Q R : Prop, P /\ Q -> Q /\ R -> P /\ R
...
Definition conj_fact_jeff : forall P Q R, P /\ Q -> Q /\ R -> P /\ R :=
  fun P Q R (H: P /\ Q) (H0: Q /\ R) =>
    match H with
    | conj HP HQ => match H0 with
                   | conj HQ' HR => conj HP HR
                   end
    end.
(*"this doesn't type check if for example I change the returned HR to HQ', 
which one should we use? isn't it better to use definitions that we know for a fact
are correct, by not type checking in cases that we make mistakes!!"*)
Compute conj_fact_jeff.

(* ===>
 = fun (P Q R : Prop) (x : P /\ Q) (x0 : Q /\ R) =>
       match x with
       | conj x1 _ => match x0 with
                      | conj _ x4 => conj x1 x4
                      end
       end
     : forall P Q R : Prop, P /\ Q -> Q /\ R -> P /\ R
*)
...
Definition or_comm_cheat : forall P Q, P \/ Q -> Q \/ P.
  intros. inversion H. right. apply H0. left. apply H0. Defined.

Compute or_comm_cheat.
Print or.
Print Logic.or.
(*
 = fun (P Q : Prop) (x : P \/ Q) =>
       match x with
       | or_introl x0 => or_intror x0
       | or_intror x0 => or_introl x0
       end
     : forall P Q : Prop, P \/ Q -> Q \/ P
...
Definition or_comm_jeff P Q : P \/ Q -> Q \/ P :=
  fun (H: P \/ Q) => match H with
                       | or_introl x => or_intror x
                       | or_intror x => or_introl x
                     end.
(*"again this only type checks if my definition is correct unlike 
or_comm that I've defined!!"*)
Compute or_comm_jeff.
...
Definition or_comm_jeff' : forall P Q, P \/ Q -> Q \/ P :=
...
Check ex_intro.
(*====>
 : forall (P : ?138 -> Prop) (x : ?138), P x -> ex P
*)
...
(*you don't need Int b/c of the detailed printed after the Print ex.*)

(*"why is this true???
shouldn't it be:
  ex_intro 4 (ev_SS 2 (ev_SS 0 ev_0)).
"*)
...
Definition ex_ev_Sn_cheat : ex (fun n => ev (S n)).
  exists 1. apply (ev_SS 0 ev_0). Defined.

Compute ex_ev_Sn_cheat.
...
Proof.
  intros X x y H P Hx. rewrite <- H. apply Hx.
(* "from cornell lecture notes:
...
(*
uesful link:
https://www.youtube.com/watch?v=1E8-LQ4l1HQ
*)
(** [] *)
