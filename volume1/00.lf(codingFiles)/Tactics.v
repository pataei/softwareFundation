...
Theorem silly_ex :
     ...
     oddb 4 = true.
Proof.
  intros H1 H2. apply H1. apply H2. Qed.
(** [] *)
...
Error: Impossible to unify "true = beq_nat n 5" with
 "beq_nat (S (S n)) 7 = true".**)
...
(**  intros l l' H. symmetry. rewrite <- rev_involutive. rewrite <- H. reflexivity. Qed.**)
(** [] *)
...
(**"the apply tactic matches on the conclusion so it will try to match the [a;b]=[e;f] 
with n = o but doesn't know what should m be"*)
...
Example trans_eq_exercise : forall (n m o p : nat),
     ...
     (n + p) = (minustwo o).
Proof.
  intros n m o p H1 H2. apply trans_eq with (m:=(n+p)).
  reflexivity. 
...
(*** "how would I name the multiple hypothesis created by the inversion??
inconsistent syntax!!! annoying!!!"
Theorem inversion_ex1' : forall (n m o : nat),
  [n; m] = [o; o] ->
  [n] = [m].
Proof.
  intros n m o H. inversion H as [| Hh, Hd]. reflexivity. Qed.
**)
...
Example inversion_ex3 : forall (X : Type) (x y z : X) (l j : list X),
  ...
  x = y.
Proof.
  intros X x y z l j H1 H2. inversion H1. inversion H2. symmetry. apply H0. Qed.
...
Theorem plus_n_n_zero: forall n:nat,
  n + n = 0 -> n = 0.
Proof.
...
Proof.
  ...
  - intros m H. simpl in H. symmetry in H. apply plus_n_n_zero in H. symmetry. apply H.
  - intros m H. destruct m as [| m'].
    + inversion H.
    + apply f_equal. apply IHn'. inversion H. rewrite <- plus_n_Sm in H1.
       rewrite <- plus_n_Sm in H1. inversion H1. reflexivity. Qed.
(***"ANY SHORTER PROOF??!?!?!?!?!?!?"**)
...
(**
Jeff and Ameya's sol:
Below is the solution to plus_n_n_injective. 
...
Proof. intros n. induction n as [| n' x].
   ...
   - intros m. induction m as [|m' h].
     + intros H. inversion H.
     + intros H. inversion H. rewrite <- plus_n_Sm in H1.
        rewrite <- plus_n_Sm in H1. inversion H1. 
        apply x in H2. rewrite ->  H2. reflexivity. Qed.
**)
(* ################################################################# *)
...
Proof.
  ...
  - intros m H2. destruct m as [| m' H3].
    + reflexivity.
    + inversion H2. 
  - intros m H4. destruct m as [| m' H4].
    + inversion H4.
    + apply f_equal. apply H1. inversion H4. reflexivity.
Qed.
(** [] *)
...
(**Theorem apply_S: forall (n m : nat),
  n = m -> S n = S m.
Proof. 
  intros n m. induction n as [| n' IH].
  - intros H. symmetry in H. apply H. reflexivity.
**)
...
Proof.
  ...
  - intros n H0. destruct n as [| n' IH2].
    + inversion H0.
    + apply IH. simpl. simpl in H0. inversion H0. reflexivity. Qed.
...
super annoying!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
error:
" Unable to find an instance for the variables B, f."**)
(** [] *)
...
(** Now we have plenty to work with: both sides of the equality are
  ...
  rewrite mult_assoc. (**"WHY DOESN'T THIS THING HAVE A DIRECTION?!?"
...
Theorem combine_split : forall X Y (l : list (X * Y)) l1 l2,
  ...
  combine l1 l2 = l.
Proof.
  intros X Y l l1 l2 H. unfold combine. destruct l1 as [| e1 l1'].
  - unfold split in H. destruct l2 as [| e2 l2'].
    + Abort. reflexivity.  simpl in H. rewrite <- H. reflexivity.
(** [] *)
...
Theorem bool_fn_applied_thrice :
  ...
  f (f (f b)) = f b.
Proof.
  intros f b. destruct (f b) eqn:Dfb.
  - destruct (b) eqn:Db.
    + rewrite -> Dfb. rewrite -> Dfb. reflexivity.
    + inversion Dfb. Abort.
(** [] *)
