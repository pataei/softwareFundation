...
(**for the types like grumble that you're passing a type var
the first arg to all its constructors is the type itself and then comes the
values of the type defined in the definition!:
d :: X:Type -> mumble -> grumble X
e :: X:Type -> X -> grumble X
the first X in e is the type itself like Int Bool etc
but the second one is a value of type X like 2 3 False ...**)

(** Which of the following are well-typed elements of [grumble X] for
    some type [X]?
      - [d (b a 5)]  --> type error! d takes a type and (b a 5) is a value
      - [d mumble (b a 5)] --> grumble mumble
      - [d bool (b a 5)] --> grumble bool
      - [e bool true] --> grumble bool
      - [e mumble (b c 0)] --> grumble mumble
      - [e bool (b c 0)] --> type error! should take a value of type bool and not grumble
      - [c]  --> mumble *)
(* FILL IN HERE *)
(** [] *)
...
Fixpoint repeat' (X: _) (x : _)  count : list X :=
  match count with
  | 0        => nil X
  | S count' => cons X x (repeat' X x count')
  end.

(** "THIS ALSO WORKS!!! INTERESTING!! AS LONG AS
YOU DEFINE X BEFORE x YOU'RE GOOD ! "
Fixpoint repeat' count X b : list X :=
  match count with
  | 0        => nil X
  | S count' => cons X b (repeat' count' X b)
  end.

"this one is also acceptable (i.e. the type var doesn't have to be 
uppercase:"
Fixpoint repeat' x b  count : list x :=
  match count with
  | 0        => nil x
  | S count' => cons x b (repeat' x b count')
  end.
...
Theorem app_assoc : forall A (l m n:list A),
  l ++ m ++ n = (l ++ m) ++ n.
Proof.
  intros A l m n. induction l as [| e l' IH].
  - reflexivity. 
  - simpl. rewrite <- IH. reflexivity.
Qed.
...
Proof.
 intros X l1 l2. induction l1 as [| e l' IH].
  - reflexivity.
  - simpl. rewrite -> IH. reflexivity.
Qed. 
(** [] *)
...
Theorem rev_involutive : forall X : Type, forall l : list X,
  rev (rev l) = l.
Proof.
  intros X l. induction l as [| e l' IH].
  - reflexivity.
...
Fixpoint combine' (X Y : Type) (lx : list X) (ly : list Y)
  ...
  | [], _ => []
  | _, [] => []
  | x :: tx, y :: ty => (x, y) :: (combine' X Y tx ty)
  end.
...
Check combine.
(** : list ?186 -> list ?187 -> list (?186 * ?187)*)
...
Check @option.
(**: Type -> Type*)
(**same because option doesn't have any implicit arguements!*)

Arguments Some {X} _.
Arguments None {X}.

Check @Some.
(**: forall X : Type, X -> option X*)
...
Proof.
  intros X Y f l1 l2. induction l1 as [| e l' IH].
  - reflexivity.
  - simpl. rewrite -> IH. reflexivity.
Qed. 

Theorem map'_app_distr : forall (X Y : Type) (f: X -> Y) (l1 l2 : list X),
...
Proof.
  intros X Y f l. induction l as [| e l' IH].
  - reflexivity.
  - simpl. rewrite -> map_app_distr, IH. reflexivity.
Qed.
(** [] *)


(***"simpl. would do the same ...!!":
Theorem map_one_elem : forall (X Y : Type) (f : X -> Y) (e : X),
  map f [e] = [f e].
  Proof. 
  intros X Y f e. simpl.
Qed.
**)
...
(* counting number of trues in a bool list e.g. *)
(** [] *)
...
Proof. reflexivity. Qed.
(**constfun 5 retruns a function that takes a nat and returns 5 no matter 
what the nat is.*)
...
(**fun e n , where e is each element of the list*)
...
Definition filter_map {X : Type} (f : X -> bool) (l : list X) : list X :=
  fold (fun x xs => if f x then x :: xs else xs) [] l.

Theorem filter_map_correct : forall X (f : X -> bool) (l : list X),
  filter_map f l = filter f l.
Proof. 
  intros X f l. induction l as [| e l' IH].
  - reflexivity.
  - simpl. rewrite <- IH. destruct (f e) as [|].
  + reflexivity.
  + Abort.
...
Proof.
  intros X Y Z f p. destruct p as [n m]; reflexivity.
Qed.
(** [] *)
