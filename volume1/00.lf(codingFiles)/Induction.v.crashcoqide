...
(*interesting that the following line doesn't need sth like:
rewrite -> plus_n_O
b/c both are 0+ blah and non of them are blah + 0!!!*)
  - simpl. reflexivity.
  - simpl. rewrite -> IHn'. reflexivity.
Qed.
...
(**  the variable names should be the vars that you're using in the main
original proof. so the following won't work:
assert (H: 0 + n' = n'). { reflexivity. }*)
  rewrite -> H.
  reflexivity.  Qed.
...
Base: n = 0 . we need to show that 0 + m = m + 0
from plus_n_O we know that m + 0 = m 
and from definition of + we know that 0 + m = m
and so 0 + m equals m + 0


hypothesis: suppose n = S n' where 
n' + m = m + n'
we need to show that 
S n' + m = m + S n'

from plus_n_Sm we get:
m + S n' = S (m + n)
from definition of + we get:
S n' + m = S (n' + m)
using the hypothesis these two expressions are equal.
...
for any n : true = beq_nat n n
which is true = n == n

    Proof: by induction on n:
base: n = 0 so
true = beq_nat 0 0
which follows from the definition of beq_nat

hypothesis: assume n = S n' where
true = beq_nat n' n'
we need to prove: true = beq_nat (S n') (S n')
from definition of beq_nat we have:
beq_nat (S n') (S n') = beq_nat n' n'
which according to the hyp is true.
*)
(** [] *)
...
Proof.
  ...
  - reflexivity.
  - simpl. 
(**"CANNOT USE A DIFFERENT VARIABLE NAME FOR N' !!! REALLY ANNOYING"**)
...
Theorem mult_1_ident : forall n : nat,
  n * 1 = n.
Proof. 
...
Theorem n_mul_Sm : forall m n : nat,
  n * S m = n * m + n.
Proof.
  intros m n. induction n as [| n' IH].
  - simpl. reflexivity.
  - simpl. rewrite -> IH. rewrite <- plus_n_Sm. rewrite <- plus_assoc. reflexivity.
Qed.
...
Proof.
  ...
  - simpl. rewrite -> IH. rewrite -> n_mul_Sm. rewrite -> plus_comm. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros n m p. intros H. induction p as [| p' IH].
...
Theorem orb_b_negbb : forall b : bool,
  orb b (negb b) = true.
Proof.
  intros b. destruct b; reflexivity.
Qed.

Theorem orb_dist : forall a b c : bool,
  orb (andb a b) c = andb (orb a c) (orb b c).
Proof.
  intros a b c. destruct a; destruct b; destruct c; reflexivity.
Qed.

Theorem orb_assoc : forall a b c : bool,
  orb a (orb b c) = orb (orb a b) c.
Proof.
  intros a b c. destruct a; destruct b; destruct c; reflexivity.
Qed.

Theorem orb_reorder : forall a b c : bool,
  orb (orb a b) c = orb (orb a c) b.
Proof.
  intros a b c. destruct a; destruct b; destruct c; reflexivity.
Qed.
...
Theorem all3_spec : forall b c : bool,
  ...
  = true.
Proof.
  intros b c. rewrite -> orb_dist. rewrite -> orb_assoc. rewrite -> orb_b_negbb. simpl.
  rewrite -> orb_assoc. rewrite -> orb_reorder. rewrite -> orb_b_negbb. reflexivity.
Qed.

Theorem helper_1 : forall n m : nat,
  n * 0 + m * 0 = n + m.
Proof. 
  intros n m. rewrite -> mult_0_r. simpl. Abort.


Theorem plus_reorder : forall n m p q : nat,
  n + m + (p + q) =  p + n + (q + m).
Proof. Admitted.
...
rewrite -> mult_0_r. rewrite -> mult_0_r. rewrite -> mult_0_r. reflexivity. 
  - rewrite -> mult_comm. simpl. rewrite -> mult_comm. rewrite -> IH. 
  rewrite -> n_mul_Sm. rewrite -> n_mul_Sm. rewrite -> plus_reorder.
  reflexivity.
Qed.
...
Theorem mult_assoc : forall n m p : nat,
  n * (m * p) = (n * m) * p.
Proof.
  intros n m p. induction n as [| n' IH1].
  - reflexivity.
...
(*** "try it again"
Theorem plus_rearragne_in_meeting: forall n m p q : nat,
  (n + m) + (p + q) = ( m + n) + (q + p).
Proof.
  intros n m p q. 
...
Theorem Sn_plus_Sn_is_SSnn : forall (n : nat),
  S n + S n = S (S (n + n)).
Proof. 
  intros n. induction n as [| n' IH].
  - reflexivity.
  - rewrite -> IH. simpl. rewrite <- plus_n_Sm. rewrite <- plus_n_Sm. reflexivity.
Qed.


Theorem bin_to_nat_pres_incr: forall (n : bin),
  bin2nat (incrBin n) = S ( bin2nat n).
Proof.
  intros n. induction n as [| n' IH | n'' IH'].
  - reflexivity.
  - reflexivity. 
  - simpl. rewrite -> IH'.  rewrite -> Sn_plus_Sn_is_SSnn. reflexivity.
Qed.
...
Fixpoint nat2bin (n : nat) : bin :=
  match n with
  | O => Ob
  | S n' => if evenb n then Db (nat2bin (div_by_2 n')) else Sb (nat2bin (div_by_2 n'))
  end.
**)
(**
