...
Proof.
  intros b. induction b; simpl;
  try (simpl; repeat rewrite optimize_0plus_sound);
  try rewrite IHb; try (rewrite IHb1; rewrite IHb2); reflexivity.
(*"why doesn't subst work?"
...
(* the simplified!!*)
    ...
    try rewrite H; try rewrite H0; try rewrite IHbevalR; 
    try rewrite IHbevalR1; try rewrite IHbevalR2;
    reflexivity.
(* case by case solution:
    + reflexivity.
    + reflexivity.
    + apply aeval_iff_aevalR' in H. apply aeval_iff_aevalR' in H0. rewrite H. rewrite H0. reflexivity.
    + apply aeval_iff_aevalR' in H. apply aeval_iff_aevalR' in H0. rewrite H. rewrite H0. reflexivity.
    + rewrite IHbevalR. reflexivity.
    + rewrite IHbevalR1. rewrite IHbevalR2. reflexivity.
*)
  - intros H; generalize dependent bv; induction b;
    simpl; intros; subst; constructor; try apply aeval_iff_aevalR'; 
...
(* case by case proof:*)
  ...
  simpl; intros; subst.
    ...
    + apply E_BNot. (* assumption. --> doesn't work b/c the goal should exactly
       match the assumption, which it doesn't here!!*) apply IHb. reflexivity.
