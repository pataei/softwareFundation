...
Proof.
(*
  apply and_intro. --> this thing doesn't 
work b/c it's expecting and conjuction which 
this is implication with a conjuction in its conclusion *)
  intros n m H. split.
(*  - induction m as [| m' IH].*)
  - destruct m as [| m'].
...
Qed.
(* "why doesn't this work?  rewrite <- plus_comm, plus_n_Sm in H." *)
(** [] *)
...
Proof.
 intros n m H.
  destruct n as [| n'].
  - left. reflexivity. (* rewrite mult_comm in H. rewrite mult_0_r in H. left. reflexivity.*)
  - right. destruct m as [| m'].
    + reflexivity. (*rewrite mult_0_r in H. reflexivity.*)
...
Proof.
  (* WORKED IN CLASS *)
  intros P contra.
(*  inversion contra. "--> this also works instead of destruct!!"*)
  destruct contra.
(*"HOW IS DESTRUCT WORKING HERE?"*)  
Qed.
...
Fact not_implies_our_not : forall (P:Prop),
  ~ P -> (forall (Q:Prop), P -> Q).
Proof.
  intros P HnP. intros Q HP. 
  unfold not in HnP. apply HnP in HP. destruct HP.
Qed.
(** [] *)
...
(*"why not use apply H instead of destruct H?
since that works too!!
this would also work:  inversion H.
here's the reason:
False is an empty inductive data type, i.e. it has no possible values, 
see here. True is an inductive data type with a single value I.
When we destruct a value X of inductive data type we 
replace the current goal with multiple subgoals, 
one per each possible value of X. 
When we destruct False, we end up with zero subgoals 
to prove (as it has zero values), therefore proof is completed.
...
Proof.
  intros P Q H.
  unfold not. intros HnQ HP. apply H in HP. apply HnQ in HP. apply HP.
Qed.
(** [] *)
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof.
  intros P. unfold not. intros [HP H]. apply H in HP. apply HP.
Qed.
(** [] *)
...
Proof.
  (* WORKED IN CLASS *)
(*  intros n. intros H. destruct H as [m Hm].*)
...
Proof.
  intros X p. intros H. unfold not.
  intros EH. destruct EH as [x E].
  apply E. apply H. 
Qed.
(** [] *)
...
Proof.
  intros X P Q. intros [x [HP HQ]]. split. 
    + exists x. apply HP.
    + exists x. apply HQ.
Qed.
...
Proof.
  ...
  - (* l = nil, contradiction *)
    simpl. (*intros H. destruct H.*) intros [].
(*do not ask!!
"WHAT IS intros [] DOING?
it introduces the cases for False, which are none and destruct in place!" *)
  - (* l = x' :: l' *)
    simpl. intros [H | H].
...
Proof.
  ...
  - intros H. induction l as [| n l' IH]. 
    + inversion H. (*simpl in H. inversion H. "don't even need simpl"*)
    + (*simpl. simpl in H.*) inversion H.
      * exists n. split. { apply H0. } { left. reflexivity. }
      * apply IH in H0. inversion H0. inversion H1. exists x.
        split. { apply H2. } { right. apply H3. }
  - intros H. inversion H. inversion H0. rewrite <- H1. apply In_map, H2.
Qed.
(** [] *)
...
Proof.
  ...
  - intros H. induction l as [| n l'' IH]. 
    + right. apply H. (* simpl in H. *) 
    + simpl in H. simpl. inversion H.
      * left. left. apply H0.
      * apply IH in H0. apply or_assoc. right. apply H0.
  - intros [H | H']. induction l as [| n l'' IH].
    + inversion H. 
    + inversion H. 
      * (*simpl. *) left. apply H0.
      * (*simpl. *) apply IH in H0. right. apply H0.
...
Proof.
  ...
  - intros H. induction l as [| n l' IH].
    ...
    + simpl. split.
      ...
      * apply IH. intros x H'. apply H. (* simpl.*) right. apply H'.
  - intros H. intros x. intros H'. induction l as [| n l' IH].
    + (*simpl in H. simpl in H'.*)  inversion H'.
    + inversion H. (*simpl in H'.*) inversion H'.
      * apply IH. { apply H1. } { simpl in H'. Abort.
(* rewrite <- H2. rewrite <- H2 in IH. apply IH in H0. rewrite H2 in H0. rewrite H2 in H0. apply IH in H0.*)
(** [] *)
...
intros n. induction n as [| n' IH].
  ...
  - rewrite evenb_S. inversion IH. destruct (evenb _). 
(* "DON'T FORGET THE DISTRUCT!!!! THX Jeff"*)
