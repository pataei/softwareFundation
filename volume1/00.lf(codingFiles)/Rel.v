...
Theorem total_relation_not_partial : 
  ~ (partial_function some_rel).
Proof.  
  unfold not. unfold partial_function.
  intros H. assert (0 = 1) as None. {
    apply H with ( x := 0).
      - apply some_rel_eq.
...
Theorem empty_relation_partial :
  partial_function empty_rel.
Proof.
  unfold partial_function. intros x y1 y2 H1 H2.
  inversion H1. inversion H2. Abort.
...
Proof.
  ...
  induction o as [| o'].
  - inversion Hmo.
  - (*"I don't like this:( anyone has any other solution??"*)
    induction Hmo as [| m' Hm'o].
    + apply le_S, Hnm.
    + apply le_S, IHHm'o.
Qed.
...
(* induction n as [| n'].
    + apply le_S, IHo'. 
apply le_S. apply IHo'. apply le_S in Hmo.*)
...
Proof.
  ...
  assert (1 <= 0). {
  apply H with (a := 0). apply le_S, le_n. }
  inversion H0.
Qed.
(** [] *)
...
Theorem Sn_le_n_false : forall n : nat, le (S n) n -> False.
Proof.
  intros n H. induction n as [| n' IH].
  - inversion H.
  - apply IH. apply le_S_n in H. apply H.
Qed.
...
Proof.
  ...
  intros n m h; destruct h as [| m0 H].
  - trivial.
  - intros H1.
  assert (S m0 <= m0). {
  ...
  + apply H1.
  + apply H. }
  apply Sn_le_n_false in H0. inversion H0.
Qed.

(*"discuss:
1) forward proving in coq? can do with assert
2) inversion on (S m <= m) isn't working, why?
3) inversion on (Sm = m) isn't working, why?
4) what omega is?
5) auto with arith?
6) absurd was intersting!
"*)

(* "from:
http://flint.cs.yale.edu/cs428/coq/library/Coq.Arith.Le.html
"
inversion H0.
  * 
absurd (S m0 <= m0). 
...
*)
(* "from:
https://stackoverflow.com/questions/27050593/how-to-prove-antisymmetric-in-coq
"
...
*)
  ...
  (*intros a b H1 H2. inversion H1; inversion H2; omega. "how the hell this is working?!?!? google omega"*)


(*  induction H1 as [| n].
  - trivial.
  - apply le_trans ((S n) a n H2 H1). in H


 induction H2 as [].
    + trivial.
    + apply le_Sn_le in H1. apply le_Sn_le in H2. Abort.
...
Proof.
  intros n m p H1 H2.
  unfold lt in H1. apply le_S_n. apply le_trans with m.
  - apply H1.
  - apply H2.
Qed.
...
Lemma rsc_trans :
  forall (X:Type) (R: relation X) (x y z : X),
      ...
      clos_refl_trans_1n R x z.
Proof.
  intros X R x y z H. induction H.
  - intros H'. apply H'.
  - intros H'. apply IHclos_refl_trans_1n in H'. apply (rt1n_trans R x y z H H').
Qed.
...
Proof.
  ...
  - intros H. induction H.
    + apply (rsc_R X R x y H). (* apply rsc_R, H. *)
    + apply rt1n_refl.
    + apply (rsc_trans X R x y z IHclos_refl_trans1 IHclos_refl_trans2).
  - intros H. induction H.
    + apply rt_refl.
    + apply rt_trans with y. apply (rt_step R x y H). apply IHclos_refl_trans_1n.
Qed.
(** [] *)
