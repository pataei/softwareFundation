...
Theorem ev_0_ex : ev 0.
Proof. apply ev_0. Qed.

Theorem ev_2 : ev 2.
Proof. apply (ev_SS 0). apply ev_0. Qed.
...
Proof. apply (ev_SS 2 (ev_SS _ ev_0)). Qed.
*)
...
Theorem plus_n_m_Zero : forall n m, n + m = 0 -> n = 0 /\ m = 0.
Proof. 
  intros n m H. induction n as [| n' IH].
  - simpl in H. split. { reflexivity. } Abort.


Theorem ev_dist_plus : forall n m, ev (n + m) <-> (ev n) /\ (ev m).
Proof.
  intros n m. split.
...
Proof.
  intros n m Hn Hm. induction Hn. (*"it's important whether you use
induction, inversion, or destruct"*)
  - apply Hm.
  - apply (ev_SS (n+m) IHHn).
Qed.
(**)
...
Theorem ev_ev__ev : forall n m,
  ev (n+m) -> ev n -> ev m.
Proof.
  intros n m H Hn. induction Hn.
  - apply H.
  - inversion H. apply IHHn. apply H1.
Qed.
(** [] *)
...
Proof.
  intros n m p Hnm. apply ev_ev__ev. rewrite <- plus_assoc. rewrite (plus_comm p (m+p)).
  rewrite plus_assoc. rewrite plus_assoc. rewrite <- (plus_assoc (n+m) _). apply ev_sum.
  - apply Hnm.
  - apply ev_even_iff. exists p. symmetry. apply double_plus.
Qed.
(** [] *)
...
Proof.
  (* WORKED IN CLASS *)
  apply le_S. apply le_S. apply (le_S 3 3 (le_n 3)).  Qed.
(* book's solutino: apply le_S. apply le_S. apply le_S. apply le_n. Qed.*)
...
Theorem test_lt2 : 2 < 4.
Proof.
  apply (le_S 3 3 (le_n 3)).
Qed.
...
(** Define an inductive binary relation [total_relation] that holds
 ...
 Inductive some_rel : nat -> nat -> Prop :=
  | some_rel_eq : forall n, some_rel n n
  | some_rel_l : forall n m, le n m -> some_rel n m
  | some_rel_m : forall n m, le n m -> some_rel m n.

Theorem test_some_rel1 : 
  some_rel 3 3.
Proof.
  apply some_rel_eq.
Qed.

Theorem test_some_rel2 :
  some_rel 3 4.
Proof.
  apply some_rel_l. apply le_S, le_n.
Qed.

Theorem test_some_rel3 :
  some_rel 4 3.
Proof.
  apply some_rel_m.  apply le_S. apply le_n.
Qed.
...
Inductive empty_rel : nat -> nat -> Prop :=
  | emp : forall n m, n > 0 /\ (n + m < 0) -> empty_rel n m.
...
Theorem test : forall n m,
  S n <= S m -> n <= m.
Proof.
Admitted.

(* "applying le_S and le_n do the same you idiot"*)
Theorem n_le_Sn : forall n,
  n <= S n.
Proof. 
  intros n. apply le_S, le_n.
Qed.
(* old stupid proof:
induction n as [| n' Hn].
  - apply O_le_n.
  - apply n_le_m__Sn_le_Sm, Hn.
Qed.
*)

Theorem Sn_le_Sm__n_le_m_Jeff's : forall n m,
  S n <= S m -> n <= m.
Proof.
  intros. induction m as [].
(* Will
  - inversion H. apply le_n. inversion H1.
  - inversion H. apply le_n. apply IHm' in H1. *)
  - inversion H.
    + trivial.
    + inversion H1.
  - inversion H.
 ...
 induction n as [| n' Hn].
  ...
  - induction m as [| m' Hm].
    + intros H. inversion H.
       assert (H': S n' <= S (S n')).
       { apply (n_le_Sn (S n')). }
       apply (le_trans (S n') (S (S n')) 0 H' H1).
    + intros H. Abort.

(*intros H. "this should work why isn't it working???!!!" apply (n_le_m__Sn_le_Sm ) in H.*)
(*    apply (n_le_m__Sn_le_Sm (S n') m) in H."b/c you can use that in your goal so it just
changes your goal to the hyp of the imlication you're using. but when you use it in the hypothesis
you have in your context it should add a new subgoal which will be the same as what you already
have which won't get you where you want?!?!"*) 
(* "maybe the hypothesis should be:???" Hn : S n' <= S (S m) -> n' <= S m *)
(*    apply test in H.
   induction m as [| m' Hm].
    + apply n_le_m__Sn_le_Sm in H. inversion Hn.
    inversion H.
    + apply le_n.
    + Abort.*)
...
Proof.
  ...
  - apply O_le_n.
  - simpl. apply n_le_m__Sn_le_Sm, IH.
Qed.
...
Proof.
 unfold lt.
  ...
  - assert (H' : S n1 <= S (n1 + n2)).
    { apply n_le_m__Sn_le_Sm. apply le_plus_l. }
    apply (le_trans (S n1) (S (n1 + n2)) m H' H).
  - rewrite plus_comm in H. assert (H' : S n2 <= S (n2 + n1)).
     { apply n_le_m__Sn_le_Sm. apply le_plus_l. }
    apply (le_trans (S n2) (S (n2 + n1)) m H' H).
Qed.
...
Proof.
  ...
  apply (le_trans (S n) m (S m) H (le_S m m (le_n m))).
Qed.
(* "first more understandable try:"
  assert (H' : m <= S m).
  { apply le_S. reflexivity. }
  apply (le_trans (S n) m (S m) H H').
Qed.*)
...
(*
  intros n m H. induction n as [| n' IH].
  - apply O_le_n.
  - assert (H': n' <= S n'). { apply le_S. reflexivity. }
...
Proof.
  intros n m. split.
  - intros H. apply leb_complete, H.
  - intros H. apply leb_correct, H.
Qed.
(** [] *)
...
Proof.
  ...
  - intros H. unfold fR. induction H.
    ...
    + rewrite plus_comm. simpl. rewrite plus_comm. rewrite IHR. trivial.
    + simpl in IHR. rewrite <- plus_n_Sm in IHR. inversion IHR. trivial.
    + rewrite plus_comm. apply IHR.
  - induction m as [| m' Hm]. Abort.
(* my attempt:
  - intros H. unfold fR in H. induction n as [| n' Hn].
    + rewrite plus_comm in H. simpl in H. rewrite H.
       induction o as [| o' Ho].
      * apply c1.
      * apply c2. apply Ho. "COMPLETE LATER!!!"
*)
...
Inductive subseq : list nat -> list nat -> Prop :=
  | sub_emp: forall l : list nat, subseq [] l
  | sub_remove_elem: forall (n : nat) (l1 l2 : list nat), subseq l1 l2 -> subseq (n::l1) (n::l2)
  | sub_remove_elem2 : forall (n : nat) (l1 l2:list nat), subseq l1 l2 -> subseq l1 (n::l2).

Theorem subseq_test1:
  subseq [1;2;3] [1;2;3].
Proof. 
  apply sub_remove_elem. apply sub_remove_elem. apply sub_remove_elem. apply sub_emp.
Qed.

Theorem subseq_test2: 
  subseq [1;2;3] [1;1;1;2;2;3].
Proof.
  apply sub_remove_elem. apply sub_remove_elem2. apply sub_remove_elem2. 
  apply sub_remove_elem. apply sub_remove_elem2. apply sub_remove_elem.
  apply sub_emp.
Qed.
...
Proof.
  ...
  - apply sub_emp.
  - apply sub_remove_elem. apply IH.
Qed.
...
Proof.
  ...
  - apply sub_emp.
  - simpl. apply sub_remove_elem. apply IHsubseq.
  - simpl. apply sub_remove_elem2. apply IHsubseq.
Qed.
...
Theorem subseq_trans : 
  forall l1 l2 l3, subseq l1 l2 -> subseq l2 l3 -> subseq l1 l3.
Proof. 
  intros l1 l2 l3 H12 H23. induction H23.
  - inversion H12. apply sub_emp. 
Abort.
(*  - "HAVE TO COMPLETE LATER!!!" apply sub_remove_elem2. apply IHsubseq.   *)
...
Proof.
  (*apply c3. apply c3.*)
  (*apply c3. apply c3.*) (* "applying c3 keeps increasing the number: 6, 7, 8, which won't help the case" *)
  (*apply c2.*) (* "cannot apply c2 b/c  you should have R (S n) (n :: l), i.e., the number should be equal to
the head of the list + 1 for you to be able to apply c2", ===> Error: Impossible to unify "3" with "5".*)
Abort.*)

Theorem r_test4:
   R 6 [3;2;1;0] -> true = false.
Proof. 
  intro H. inversion H. inversion H0.
(* "HOW CAN I PROVE THAT THIS DOESN'T HOLD?"*)
Abort.
