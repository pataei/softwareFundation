...
Print test_next_weekday. (*shows the proof used to prove your claim*)
...
(*Definition oddb (n) : bool   :=   negb (evenb n).*)

(*Notation "x .. y" := (compose x y) .
Definition oddb (n:nat) : bool   :=   negb  evenb.*)
...
Proof.
  ...
  - reflexivity.
(*why does simpl works as follows:
S n' * 0 = 0
n' * 0 = 0 
b/c S n * m simplifies to m + n * m 
*)
...
simpl.
compute.
the compute forces the simplification, just try it out.*)
...
(* could have omitted the as clause and coq will come up with a
variable name itself although this is considered generally 
BAD STYLE*)
  - reflexivity.
  - reflexivity.   Qed.
(**
remember the curry-howard morphism. 
the proof that you have is actually a value of the type and this type is 
basically your theorem. KEEP THIS IN MIND!!
so if you do:
Pring plus_1_neq_0.
you'll see the value that it's coming up with.
*)
Theorem plus_1_neq_0' : forall n : nat,
  beq_nat (n + 1) 0 = false.
Proof.
...
Theorem andb_commutative''' : forall b c, andb b c = andb c b.
Proof.
  intros b c. destruct b; destruct c; reflexivity. Qed.

(**see the problem in the following proof:
Theorem andb_commutative'''' : forall b c, andb b c = andb c b.
Proof.
...
(**in the meeting suggested by martin:, get it working
Theorem andb_true_elim2' : forall b c : bool,
  andb b c = true -> c = true.
Proof.
  intros b c. destruct b.
...
Theorem true_is_true_orb_a: 
  forall (a : bool), true = orb true a.
Proof.
  intro a. destruct a.
  - simpl. reflexivity.
  - simpl. reflexivity.
...
Theorem false_is_false_andb_a: 
  forall (a : bool), false = andb false a.
Proof.
  intro a. destruct a.
  - simpl. reflexivity.
  - simpl. reflexivity.
...
Theorem andb_eq_orb :
  ...
  b = c.
Proof.
  intros b c. intros H. destruct b.
  - rewrite -> a_is_true_andb_a. rewrite -> H. rewrite <- true_is_true_orb_a. reflexivity.
  - rewrite -> a_is_false_orb_a. rewrite <- H. rewrite <- false_is_false_andb_a. reflexivity.
Qed.
...
Fixpoint bin2nat (n : bin) : nat :=
  match n with
  | Ob => O
  | Db n' => bin2nat n' + bin2nat n'
  | Sb n' => S (bin2nat n' + bin2nat n')
(**  | Db n' => double (bin2nat n')
  | Sb n' => S (double (bin2nat n'))
**)
  end.
...
Example test_bin_incr1 : bin2nat Ob = O.
Proof. reflexivity. Qed.
Example test_bin_incr2 : bin2nat (incrBin Ob) = S O.
Proof. reflexivity. Qed.
