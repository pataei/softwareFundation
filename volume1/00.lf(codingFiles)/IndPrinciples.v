...
natlist1_ind
     : forall P : natlist1 -> Prop,
       P nnil1 -> (forall n : natlist1, P n -> forall n0 : nat, P (nsnoc1 n n0)) -> forall n : natlist1, P n
*)
...
(*my answer:
byntree_ind : forall P : byntree -> Prop, P bempty -> (forall y : yesno, P (bleaf y) ) ->
(forall (z : yesno) (b1 "b2" : byntree), P b1 "-> P b2" -> P (nbranch z b1 b2))-> forall x : byntree, P x
*)
...
ExSet_ind
     : forall P : ExSet -> Prop,
       (forall b : bool, P (con1 b)) ->
       (forall (n : nat) (e : ExSet), P e -> P (con2 n e)) -> forall e : ExSet, P e.
*)
...
tree_ind
     : forall (X : Type) (P : tree X -> Prop),
       ...
       "("forall t : tree X, P t -> forall t0 : tree X, P t0 -> P (node "X" t t0)")" -> forall t : tree X, P t
*)
...
mytype_ind
     : forall (X : Type) (P : mytype X -> Prop),
       (forall x : X, P (constr1 X x)) ->
       (forall n : nat, P (constr2 X n)) ->
       (forall m : mytype X, P m -> forall n : nat, P (constr3 X m n)) -> forall m : mytype X, P m
*)
...
foo_ind
     : forall (X Y : Type) (P : foo X Y -> Prop),
       (forall x : X, P (bar X Y x)) ->
       (forall y : Y, P (baz X Y y)) ->
       (forall f1 : nat -> foo X Y, (forall n : nat, P (f1 n)) -> forall n : nat, P (quux X Y f1 n)) ->
       forall f2 : foo X Y, P f2
"if I didn't have the () in quux it'll give me, don't mix up your right and left idiot!!!:
foo_ind
     : forall (X Y : Type) (P : foo X Y -> Prop),
       (forall x : X, P (bar X Y x)) ->
       (forall y : Y, P (baz X Y y)) ->
       (forall (n : nat) (f1 : foo X Y), P f1 -> forall n0 : nat, P (quux X Y n f1 n0)) ->
       forall f2 : foo X Y, P f2

but isn't arrows right associative? so () shouldn't really matter?!?!"
*)
...
foo'_ind
     : forall (X : Type) (P : foo' X -> Prop),
       (forall (l : list X) (f : foo' X), P f -> P (C1 X l f)) -> P (C2 "X") -> forall f1 : foo' X, P f1
*)
...
Definition plus_assoc_new : nat -> nat -> nat -> Prop := 
  fun n m p => n + (m + p) = (n + m) + p.

Definition plus_assoc_new' (n m p : nat) : Prop := 
  n + (m + p) = (n + m) + p.

Definition plus_comm_new : nat -> nat -> Prop :=
  fun n m => n + m = m + n.

Definition plus_comm_new' (n m : nat) : Prop :=
  n + m = m + n.
...
Proof.
  ...
  - unfold plus_comm_new'. simpl. rewrite <- plus_n_O. trivial.
  - unfold plus_comm_new'. intros m H. simpl. rewrite <- H. rewrite <- plus_n_Sm. trivial.
Qed.

Theorem plus_assoc_new'' : forall n m p : nat,
  plus_assoc_new n m p.
Proof.
  intros n m. apply nat_ind. 
  - unfold plus_assoc_new. rewrite <-  plus_n_O. rewrite <-  plus_n_O. trivial.
  - unfold plus_assoc_new. intros p H. rewrite <- plus_n_Sm. rewrite <- plus_n_Sm.
  symmetry. rewrite <- plus_n_Sm. rewrite H. trivial.
Qed.
...
le_ind
     : forall P : nat -> nat -> Prop,
       (forall n : nat, P n n) ->
       (forall n m : nat, le n m -> P n m -> P n (S m)) -> forall n n0 : nat, le n n0 -> P n n0
*)
