...
(**remember that the Check command does :t in haskell*)
Check (pair 3 5).
...
(* this would also work:
   intros p.  destruct p as [].  simpl.  reflexivity.  Qed. 
it's just that coq is gonna name the vars itself. 
note that coq can simplify it w.r.t. the definition of the
pair and its structure *)
...
(**
all these intros, destruct, replace, assert ... are "TACTICS"
*)
...
(**"function composition in coq: the type must be the whole type 
signature of the function and not only its output type!!
also the library:
Require Import Coq.Program.Basics.
is required for it to work!"**)
Definition countoddmembers' : natlist -> nat := 
compose length oddmembers.
...
Fixpoint count (v:nat) (s:bag) : nat := 
  match s with 
(**"note that you don't really need to pattern match on v!!"*)
  | [] => O
(** sucks that I cannot do that. why can't I do this?
  | v, v :: t => S (count v t)
  | v, _ :: t => count v t
*)
(**  | h :: t => if beq_nat v h then S (count v t) else count v t 
the following is the better definiton that results into having simpler proofs*)
...
Fixpoint remove_one (v:nat) (s:bag) : bag :=
  match v, s with 
  | v, [] => []
  | v, h :: t => if beq_nat v h then t else h :: remove_one v t
  end. 
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match v, s with 
  | v, [] => []
  | v, h :: t => if beq_nat v h then remove_all v t else h :: remove_all v t
  end.
...
(* Fixpoint subset (s:bag) (s':bag) : bool := 
  ...
  | _, [] => false
  | [], _ => true
  | h :: t, h' :: t' => if beq_nat h h' then subset t t' else subset s (t' ++ [h'])
  end. *)

(*"ASK SOMEONE!! ERIC/JEFF!!"
Fixpoint subset' (s:bag) (s':bag) : bool := 
  match s, s' with
  | [], _ => true
  | h :: t, s' => leb (count h s) (count h s') 
                      && subset' (remove_all h t) (remove_all h s')
  end.*)
...
Theorem count_add_plus_count:
  forall (x : nat) (b : bag), count x (add x b) = S (count x b).
Proof. 
  intros x b. simpl. rewrite <- beq_nat_refl. reflexivity.
Qed. 
...
Theorem tail_length_pred : forall l:natlist,
...
Proof. reflexivity.  Qed.
Example test_rev2:            reverse nil = nil.
Proof. reflexivity.  Qed.
...
(**could have applied both rewrite with the same direction at once using comma between them:
...
Proof.
  intros l1 l2. induction l1 as [| n l' IH].
  - simpl. rewrite -> app_nil_r. reflexivity.
(**always simplify "FIRST":
  - rewrite -> nil_app. simpl. rewrite -> app_nil_r. reflexivity.*)
...
(**"easier approach?Eric was sure that there must be an easier way but didn't know what!"*)
Qed.
...
Proof.
  intros l1 l2. induction l1 as [| n l' IH].
  - reflexivity.
  - destruct n as [| n']; simpl; rewrite -> IH; reflexivity.
(***    + simpl. rewrite -> IH. reflexivity.
    + simpl. rewrite -> IH. reflexivity. 
**)
Qed.
...
Proof.
  ...
  - destruct n as [| n' ].
    ...
    + simpl. rewrite -> IH. reflexivity.
Qed. 
(** how can I pattern match on n ==> ANSWERED!! NICE!!*)
(*** the following opens up the function count which I don't want to do!!
simpl. *)
(** [] *)
...
Proof.
  intros n s1 s2. induction s1 as [| n' s' IH].
  - reflexivity.
  - simpl. rewrite -> IH, plus_assoc. reflexivity.
Qed.
...
Proof.
  intros l. destruct l as [| n ns].
  - reflexivity.
  - intros default. reflexivity.
Qed.
(** [] *)
...
(* "NONE??? b/c it doesn't have a base case!!" *)
(** [] *)
